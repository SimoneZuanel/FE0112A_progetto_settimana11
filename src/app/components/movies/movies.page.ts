  ...
  template: `
    <div class="container">
      <div class="caricamento text-center">
        <div *ngIf="isLoading" class="spinner-grow text-success" style="width: 25px; height: 25px" role="status"></div>
        <div *ngIf="isLoading" class="spinner-grow text-success" style="width: 25px; height: 25px" role="status"></div>
        <div *ngIf="isLoading" class="spinner-grow text-success" style="width: 25px; height: 25px" role="status"></div>
        <h3 id="caricamento" class="text-white" *ngIf="isLoading">Caricamento...</h3>
      </div>
      <div class="row justify-content-around ">
        <div *ngFor="let movie of movies" class="card text-center m-2 p-2 shadow-lg p-3 mb-5 rounded">
          ...
          <div class="card-body">
            ...
            <h3>Rilasciato il: <strong>{{ movie.release_date }}</strong></h3>
            <h5>Lingua originale: <strong>{{ movie.original_language }}</strong></h5>
            <h3>Voto medio: <strong>{{ movie.vote_average }}</strong></h3>
      </div>
    </div>
